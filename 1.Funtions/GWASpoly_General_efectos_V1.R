...
# source("scorecalc_general.R")
# source("makefull.R")
# source("design_geneal.R")
...
GWASpoly_general <- function(data,models,traits=NULL,params=NULL,n.core=1,quiet=F) {
  ...
  all.effects_general <- all.effects
  ...
  for (j in 1:n.trait) {
    ...
    if (params$P3D) {
      ...
      data2 <- mapply(function(x,y){list(markers=x,Hinv=y)},
                      x=markers,y=tmp,SIMPLIFY=FALSE)
    ...
    for (k in 1:n.model) {
      ...
      if (params$P3D) {  
        if (n.core > 1) {
          score.list <- parLapply(cl,X=data2,fun=function(x,other){.score.calc_general(data@geno[,x$markers,drop=FALSE],y=other$y,Z=other$Z,X=other$X,K=NULL,Hinv=x$Hinv,ploidy=other$ploidy,model=other$model,min.MAF=other$min.MAF,max.geno.freq=other$max.geno.freq)},other=list(y=y,Z=Z,X=X2,ploidy=data@ploidy,model=models[k],min.MAF=params$min.MAF,max.geno.freq=params$max.geno.freq))
        } else {
          score.list <- lapply(data2,function(x,other){
            .score.calc_general(data@geno[,x$markers,drop=FALSE],y=other$y,Z=other$Z,
                        X=other$X,K=NULL,Hinv=x$Hinv,ploidy=other$ploidy,
                        model=other$model,min.MAF=other$min.MAF,
                        max.geno.freq=other$max.geno.freq)},
            other=list(y=y,Z=Z,X=X2,ploidy=data@ploidy,
                       model=models[k],min.MAF=params$min.MAF,
                       max.geno.freq=params$max.geno.freq))
        }
      } else {
        if (n.core > 1) {
          score.list <- parLapply(cl,X=data2,fun=function(x,other){.score.calc_general(data@geno[,x$markers,drop=FALSE],y=other$y,Z=other$Z,X=other$X,K=x$K,Hinv=NULL,ploidy=other$ploidy,model=other$model,min.MAF=other$min.MAF,max.geno.freq=other$max.geno.freq)},other=list(y=y,Z=Z,X=X2,ploidy=data@ploidy,model=models[k],min.MAF=params$min.MAF,max.geno.freq=params$max.geno.freq))
        } else {
          score.list <- lapply(data2,function(x,other){.score.calc_general(data@geno[,x$markers,drop=FALSE],y=other$y,Z=other$Z,X=other$X,K=x$K,Hinv=NULL,ploidy=other$ploidy,model=other$model,min.MAF=other$min.MAF,max.geno.freq=other$max.geno.freq)},other=list(y=y,Z=Z,X=X2,ploidy=data@ploidy,model=models[k],min.MAF=params$min.MAF,max.geno.freq=params$max.geno.freq))
        }
      }
      scores[,k] <- unlist(lapply(score.list,function(el){el$score}))
      betas[,k] <- unlist(lapply(score.list,function(el){el$beta}))
      generl = (lapply(score.list,function(el){el[["gener"]]}))
    } 
    all.scores[[j]] <- data.frame(scores,check.names=F)
    all.effects[[j]] <- data.frame(betas,check.names=F)
    all.effects_general[[j]] <- (generl)
  }
  ...
  return(new("GWASpoly.fitted",map=data@map,pheno=data@pheno,fixed=data@fixed,
             geno=data@geno,ploidy=data@ploidy,K=data@K,scores=all.scores,
             effects=all.effects,gene_effec = all.effects_general,params=params))  
}